...
class MascotNanaiApp {
  ...
  async scanAndLoadFirstGhost() {
    ...
    try {
      ...
      if (this.ghosts.length > 0) {
        ...
        this.showBalloon(
          "ゴーストファイルが見つかりません。assets/ghostディレクトリを確認してください。"
        );
  ...
  setupEventListeners() {
    ...
    const menuIframeContainer = document.getElementById(
  ...
  async sendRandomMessageToGhost() {
    // 利用可能なSHIORIイベント
    const randomEvents = ["OnBoot", "OnSecondChange", "OnTalk", "OnAITalk"];
  ...
  updateGhostCharacter(ghostName) {
    ...
    if (this.elements.ghostCharacter) {
      const placeholder = this.elements.ghostCharacter.querySelector(
  ...
  applyGhostSize() {
    ...
    if (placeholder) {
      ...
      placeholder.style.fontSize =
        sizes[this.settings.ghostSize] || sizes.medium;
    }
  }
  ...
  toggleIframeMenu(menuContainer) {
    ...
    console.log(
      `iframeメニュー表示切り替え: ${isCurrentlyVisible ? "非表示" : "表示"}`
    );
  ...
  toggleMenu(menu) {
    ...
    console.log(
      `メニュー表示切り替え: ${isCurrentlyVisible ? "非表示" : "表示"}`
    );
  ...
  showModal(contentId) {
    ...
    if (!content || !this.elements.modalContent || !this.elements.modalOverlay)
      return;
  ...
  setupGhostModalListeners() {
    document
      .getElementById("refresh-ghosts-btn")
  ...
  async refreshGhosts() {
    ...
    try {
      const ghostDirectory =
        document.getElementById("ghost-directory")?.value || "assets/ghost";
      ...
      this.updateGhostStatus(
        `${this.ghosts.length}個のゴーストが見つかりました`
      );
  ...
  refreshGhostList() {
    ...
    if (this.ghosts.length === 0) {
      ghostList.innerHTML =
        '<div class="info">ゴーストが見つかりません。パスを確認してスキャンしてください。</div>';
      return;
    }

    ghostList.innerHTML = this.ghosts
      .map(
        (ghost, index) =>
          `<div class="ghost-item ${
            this.currentGhost?.name === ghost.name ? "active" : ""
          }" 
  ...
  setupBalloonModalListeners() {
    document
      .getElementById("refresh-balloons-btn")
      ?.addEventListener("click", () => {
  ...
  setupTestModalListeners() {
    document
      .getElementById("test-shiori-init")
      ?.addEventListener("click", () => {
        this.testShioriInit();
      });

    document
      .getElementById("test-shiori-request")
      ?.addEventListener("click", () => {
        this.testShioriRequest();
      });

    document
      .getElementById("test-shiori-finalize")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("test-path-resolve")
      ?.addEventListener("click", () => {
  ...
  saveSettings() {
    // UI要素から設定を取得
    this.settings.autoLoadGhost =
      document.getElementById("auto-load-ghost")?.checked || false;
    ...
    this.settings.ghostSize =
      document.getElementById("ghost-size")?.value || "medium";
    this.settings.debugLevel =
      document.getElementById("debug-level")?.value || "info";

    // ローカルストレージに保存
    localStorage.setItem(
      "mascot-nanai-settings",
  ...
  setupDebugModalListeners() {
    document
      .getElementById("apply-log-level")
      ?.addEventListener("click", () => {
        this.applyLogLevel();
      });
  ...
  clearLogs() {
    ...
    if (logOutput) {
      logOutput.innerHTML =
        '<div class="log-entry info">ログをクリアしました</div>';
    }
  }
  ...
  exportLogs() {
    ...
    if (logOutput) {
      ...
      a.href = url;
      a.download = `mascot-nanai-logs-${new Date()
        .toISOString()
        .slice(0, 10)}.txt`;
      a.click();
