  ...
  searchGhosts(query) {
    ...
    return this.ghosts.filter(
      (ghost) =>
        ...
        (ghost.description &&
          ghost.description.toLowerCase().includes(lowercaseQuery)) ||
        (ghost.craftman &&
          ghost.craftman.toLowerCase().includes(lowercaseQuery))
    );
  }
...
globalThis.shioriManager = new ShioriManager();

// デバッグ用の便利関数をグローバルに追加
globalThis.debugShiori = {
  /**
   * デバッグ用ゴーストスキャン
   */
  async scanDefault() {
    try {
      ...
      const result = await globalThis.shioriManager.scanGhostDirectory(
        "./assets/ghost"
      );
   ...
   */
  async testEvents() {
    const manager = globalThis.shioriManager;
   ...
   */
  showStatus() {
    const status = globalThis.shioriManager.getStatus();
    console.table(status);

    const typeCount = globalThis.shioriManager.getGhostsByType();
...
console.log("使用方法:");
console.log("  globalThis.shioriManager - SHIORIマネージャー");
console.log("  globalThis.debugShiori - デバッグ用ヘルパー");
